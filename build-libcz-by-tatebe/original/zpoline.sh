...
export PATH=/work/0/NBB/tatebe/zpoline/bin:$PATH
...
export LIBZPHOOK=/work/0/NBB/tatebe/zpoline/lib/libcz.so

MPIENVS="-x PATH"
MPIENVS="$MPIENVS -x CHFS_SERVER -x CHFS_CHUNK_SIZE -x CHFS_ASYNC_ACCESS -x CHFS_RPC_TIMEOUT_MSEC"
MPIENVS="$MPIENVS -x LD_LIBRARY_PATH -x LIBZPHOOK -x LD_PRELOAD=/work/0/NBB/tatebe/zpoline/lib/libzpoline.so"
...
mpirun $MPIARGS $MPIENVS ior -C -t $CHFS_CHUNK_SIZE -b $BSIZE -F \
	-a POSIX \
